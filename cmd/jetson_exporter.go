...
func main() {
	var (
		webConfig      = kingpinflag.AddFlags(kingpin.CommandLine, ":9102")
	...
	logger.Info("Starting jetson-exporter", "tegraPath", *tegrastatsPath)
	...
	go func() {
		...
		if err != nil {
			logger.Error("Can't find tegrastats")
			panic(err)
		}
		...
		for scanner.Scan() {
			...
			if err != nil {
				logger.Error("Error parsing tegrastats, provide this log in a Github Issue for resolution. https://github.com/GDWR/jetson-exporter/issues/new",
					"content", content,
					"err", err)
				continue
			}
